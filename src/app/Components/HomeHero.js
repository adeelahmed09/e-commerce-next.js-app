...
function HomeHero() {
  ...
  return (
    <div id="textContainer" onMouseMove={(e)=>{onMouseMoveHandler(e)}} className="w-full flex h-screen relative sm:px-20 px-5 py-5 flex-col sm:justify-center ">
      <img src="/HeroBg.png" className="w-screen  h-screen sm:flex hidden right-0 -z-0  brightness-70 object-right-top object-cover  absolute top-0" alt="" />
      <div ref={container} id="text" className="z-10 absolute w-fit ">
        ...
        <Link href={"#"} className="z-10 sm:text-xl text-md mt-2 bg-[#0A84FF] w-fit px-3 py-2 rounded-lg flex justify-center items-center font-semibold text-[#e6e6e6] ">Shop Now</Link>
      </div>
      <img src="/HeroBgMobile.png" className=" top-0 sm:hidden absolute w-screen object-cover left-0 brightness-70 -z-0 h-screen" alt="" />
    </div>
  )
}
